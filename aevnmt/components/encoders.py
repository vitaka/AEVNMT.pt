...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, input_size, num_heads, num_layers, dim_ff, dropout=0.,z_size=0):
        super().__init__()
        self.pos_enc = PositionalEncoding(input_size, dropout)
        encoder_layer = nn.TransformerEncoderLayer(d_model=input_size+z_size,
                                                   nhead=num_heads,
                                                   dim_feedforward=dim_ff,
                                                   dropout=dropout)

        self.transformer_enc = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.input_size = input_size+z_size

    def forward(self, x_embed, seq_len, z=None):
        ...
        if z is not None:
            #z: [B, latent_size]
            x_embed = torch.cat([x_embed, z.unsqueeze(0).repeat( x_embed.size(0) ,1,1) ],dim=-1)
